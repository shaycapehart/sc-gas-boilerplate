 ...
 *====================================================================================================================================*/
  ...
  export function parseJSONObject(
    ...
    transformData_(data, options, defaultTransform);
  ...
  function transformData_(data, options, transformFunc) {
    for (var i = 0; i < data.length; i++) {
      for (var j = 0; j < data[0].length; j++) {
        defaultTransform(data, i, j, options);
      }
    }
  }
   ...
   *    debugLocation: Prepend each value with the row & column it belongs in
   */
  function defaultTransform(data, row, column, options) {
  ...
  function getDataFromNamedSheet_(sheetName) {
    ...
    var jsonRange = !sheetName
      ? ss.getActiveRange()
      : ss.getSheetByName(sheetName).getDataRange();
